...
def generar_contexto_club(db: Session, club_id: int):
    ...
    if torneo:
        ...
        info_torneo = f"TORNEO ACTIVO: '{torneo.name}' (Categoría: {cat}). Estado: {torneo.status}. Inscritos: {inscritos}."
...
def debug_db(db: Session = Depends(get_db)):
    ...
    lista_jugadores = [{"id": p.id, "nombre": p.name, "cat": getattr(p, 'category', 'N/A')} for p in jugadores]
