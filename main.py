...
async def ver_club(request: Request, club_id: int, db: Session = Depends(get_db)):
    ...
    if torneo and torneo.status == "inscription":
        ...
        # DIAGNÓSTICO: Ver qué está leyendo la web
        datos = torneo.smart_data if torneo.smart_data else {}
        ids = datos.get("inscritos", [])
        print(f"👀 WEB LEYENDO: Torneo {torneo.name}, IDs inscritos: {ids}")
        
        if ids:
            # Truco: Traerlos en el orden que se inscribieron (o por ELO)
        ...
        if 'messages' in entry:
            ...
            if accion == 'crear_jugador':
                ...
                if jugador and torneo:
                    ...
                    # 1. Obtener copia fresca de los datos
                    ...
                    if jugador.id not in lista_inscritos:
                        ...
                        # 2. Reasignar y marcar como modificado
                        torneo.smart_data = datos_actuales
                        flag_modified(torneo, "smart_data") # ¡ESTO ES CLAVE!
                        
                        db.add(torneo) # Asegurar que está en sesión
                        db.commit()
                        db.refresh(torneo) # Confirmar que se guardó
