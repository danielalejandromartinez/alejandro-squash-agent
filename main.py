...
def enviar_whatsapp(telefono_destino, mensaje):
    ...
    try:
        print(f"📤 Enviando a Facebook...")
        response = requests.post(url, headers=headers, json=data)
        print(f"👉 Facebook dice: {response.status_code} - {response.text}")
    except Exception as e:
        print(f"❌ Error de conexión con Facebook: {e}")
...
def generar_contexto_club(db: Session):
    ...
    if torneo:
        ...
        info_torneo = f"TORNEO ACTIVO: '{torneo.name}'. Inscritos: {inscritos}."
    
    top_players = db.query(Player).order_by(Player.elo.desc()).limit(3).all()
    ranking_txt = ", ".join([f"{p.name} ({p.elo})" for p in top_players])
    
    return f"- {info_torneo}\n- Top 3 Ranking: {ranking_txt}"

# --- RUTAS ---
@app.on_event("startup")
def startup_event():
    # Asegurar que existe al menos un club
    db = SessionLocal()
    club = db.query(Club).first()
    if not club:
        print("⚠️ Creando Club Demo al inicio...")
        ...
        if 'messages' in entry:
            ...
            if accion == 'crear_jugador':
                ...
                if not existe:
                    ...
                    nuevo = Player(name=nombre, owner_id=padrino.id, club_id=club_demo.id)
                    db.add(nuevo); db.commit()
                    await manager.broadcast("update")

            elif accion == 'crear_torneo':
                print("🏆 Entrando a lógica de crear torneo...")
